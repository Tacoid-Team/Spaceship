...
public class GameScreen extends AbstractGameScreen {
	...
	protected void step(float delta) {
		...
		double angleAbs = (Math.abs(spaceship.getBody().getAngle()) % Math.PI); 
		if (!spaceship.areBothOn() && (angleAbs < 0.5 || angleAbs > Math.PI - 0.5) && spaceship.getBody().getLinearVelocity().len2() < 1) {
			if (landDate == 0) {
	...
	private boolean closeToEnd() {
		return (spaceship.getX() - end_x * WORLD_TO_BOX) * (spaceship.getX() - end_x * WORLD_TO_BOX) + (spaceship.getY() - end_y * WORLD_TO_BOX) * (spaceship.getY() - end_y * WORLD_TO_BOX) < (50 * WORLD_TO_BOX) * (50 * WORLD_TO_BOX);
	}
	...
	public void beginContact(Contact contact) {
		...
		if (contact.getFixtureA().getBody() == spaceship.getSpaceShipBody()
			...
			boolean base = isSpaceshipBase(contact);
	...
	private boolean isSpaceshipBase(Contact contact) {
